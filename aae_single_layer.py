...
def model_generator(latent_dim, input_shape, hidden_dim=1000):
    return Sequential([
        Dense(input_shape[0], name="generator_h1", input_dim=latent_dim, activation='sigmoid')],
        name="generator")
        
def model_encoder(latent_dim, input_shape, hidden_dim=1000):
    x = Input(input_shape, name="x")
    z = Dense(latent_dim, name="encoder_h1", activation='tanh')(x)
...
def model_discriminator(input_shape, output_dim=1, hidden_dim=1000):
    z = Input(input_shape)
    h = z
    y = Dense(output_dim, name="discriminator_h1", activation="sigmoid")(h)
    return Model(z, y)
